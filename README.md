...
## Изменения после прохождения в финал

    - дашборд добавлены результаты работы алгоритма по 40 заявкам
    - произвели расчет алгоритма Монте-Карло на 10000 итераций
    - сделали правки в презентации
    - добавили свойство у таблицы заявок, отображающее необходимость использования ледоколов
    - добавили примеры работы алгоритма в виде анимации
...
- `tests/` - тесты
- `dump` - дамп базы данных и дашборда

## Инструкция по установке

Для запуска проекта необходимо содержимое каталога `deploy`, каталог `app`, каталог `scripts` и файл `requirements.txt` разместить в корневой директории, далее следовать инструкциям из файла README.md находящегося в каталоге `deploy`
